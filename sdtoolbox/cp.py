...
###############################################################################
Theory, numerical methods and applications are described in the following
report:
...
###############################################################################
Formatted according to PEP by Alexander Drakon
"""
...
class CPSys(object):
    ...
    def __call__(self, t, y):
        """
        Evaluates the system of ordinary differential equations for an
        adiabatic, constant-pressure, zero-dimensional reactor.
        It assumes that the 'gas' object represents a reacting ideal gas
        mixture.
...
def cpsolve(gas,
            ...
            relTol=1e-5, absTol=1e-8, Method='LSODA'):
    """
    ...
    out = solve_ivp(CPSys(gas), tel, y0, method=Method,
